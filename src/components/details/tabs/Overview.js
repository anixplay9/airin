...
function Overview({data}) {
    ...
    const getAnixPlaygTime = (AnixPlaygdate) => {
        const timeDifference = AnixPlaygdate * 1000 - Date.now();
    ...
    const getAnixPlaygTimeUnix = (seconds) => {
                ...
                <div className={styles.detailscontent}>
                  {data?.status==='RELEASING' && 
                    <div className={styles.singlecontent}>
                        <span className={`${styles.sideheading} font-semibold !text-[15px]`}>AnixPlayg</span>
                        <Tooltip content={getAnixPlaygTimeUnix(data?.nextAnixPlaygEpisode?.AnixPlaygAt)}
                        // showArrow
                        size='sm'
                        classNames={{
                            base: [
                            ...
                            <span className={`${styles.con} !text-white cursor-pointer`} suppressHydrationWarning>EP {data?.nextAnixPlaygEpisode?.episode}: {getAnixPlaygTime(data?.nextAnixPlaygEpisode?.AnixPlaygAt)}</span>
                        </Tooltip>
                    </div>}
                    <div className={styles.singlecontent}>
                    ...
                    <div className={styles.singlecontent}>
                        <span className={styles.sideheading}>Episodes</span> <span className={styles.con}>{data?.episodes || data?.nextAnixPlaygEpisode?.episode - 1 || "?"}</span>
                    </div>
                    <div className={styles.singlecontent}>
