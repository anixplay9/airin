...
function Searchcard({ searchvalue, selectedYear, seasonvalue, formatvalue, genrevalue, sortbyvalue, AnixPlaygvalue }) {
    ...
    useEffect(() => {
        ...
        fetchsearch();
    }, [debouncedSearch, selectedYear, seasonvalue, formatvalue, genrevalue, sortbyvalue, currentPage, AnixPlaygvalue]);
                            ...
                            <div key={item.id} className={styles.carditem}>
                                ...
                                <div className="hidden xl:flex h-[85%] w-[100%] rounded absolute hover:bg-gradient-to-t from-black/90 to-transparent z-7 opacity-0 hover:opacity-100 transition-all duration-300 ease  justify-center">
                                    <div className="bottom-4 absolute text-xs font-light flex flex-wrap items-center justify-center gap-[.3rem] z-10">
                                        ...
                                        <span>Ep {item.episodes || item?.nextAnixPlaygEpisode?.episode-1 || '?'}</span>
                                    </div>
                                </div>
